...
const ExpanseTableData = () => {
    ...
    const { data, isLoading, isFetching, refetch } = useFetchApi(
        `expanseData_page_${offset}`,
        ENDPOINTS.expanse.list,
        { limit, offset }
    );

    const { mutate: saveExpanse } = usePostApiRequest(ENDPOINTS.expanse.payment, 'expanseSave');
    ...
    return (
        <>
            <AccountingMasterTemplate
                ...
                hasMore={hasMore}
                onSave={handleSave}
                onDelete={openDelete} // Prop connected to open modal
                addPagePath="/accounting/expanse"
            />
...
export default ExpanseTableData;